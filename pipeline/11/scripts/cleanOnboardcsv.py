...
output_file_name_and_path = "../MISSION_DATA/A11_onboard_cleaned.csv"
...
inputFilePath = "../MISSION_DATA/onboard.csv"
reader = csv.reader(open(inputFilePath, "rU"), delimiter='|')
for row in reader:
    outputFile = open(output_file_name_and_path, "a")
    curReadRow += 1
    # print(row)
    if row[2].startswith("Tape") and not row[2].startswith("Tape recorder") : # print tape row
        tapeNumber = row[3]
        pageCounter += 1
    ...
    elif row[1] == '' and not row[2].startswith("BEGIN LUNAR") and not row[2].startswith("REST PERIOD"):
        ...
        if not curTimestamp > lastTimestamp and curTimestamp != '':
