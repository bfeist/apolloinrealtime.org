...
# Define paths
CSV_PATH = "D:/Apollo_11/_website/_webroot/11/MOCRviz/data/tape_ranges.csv"
INPUT_DIR = "F:/A11_whisperx_MOCR_transcription"  # Both input and output
OUTPUT_DIR = "F:/A11_whisperx_MOCR_transcription"  # Both input and output
...
for tape_folder in os.listdir(INPUT_DIR):
    ...
    if not tape_folder.startswith("T"):
        continue
    ...
    for filename in os.listdir(tape_path):
        ...
        # Parse filename
        # Expected format: defluttered_A11_T708_HR1L_CH12_whisperx_aligned_transcript.json
        parts = filename.rstrip(".json").split("_")
        if len(parts) < 4:
            print(f"Filename {filename} does not match expected format. Skipping.")
            continue

        tape_number = parts[2]  # e.g., T708
        tape_type = parts[3]  # e.g., HR1L
        channel = parts[4]  # e.g., CH12
