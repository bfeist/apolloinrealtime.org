...
# Define paths
INPUT_DIR = "F:/A11_whisperx_MOCR_transcription"
...
for tape_folder in os.listdir(INPUT_DIR):
    ...
    for filename in os.listdir(tape_path):
        ...
        if len(parts) < 4:
            ...
            continue
        tape_type = parts[3].upper()  # e.g., 'HR1L', 'HR2U'
...
all_channels_filepath = os.path.join(OUTPUT_DIR, "A11_all_channels.txt")
try:
    ...
    print(f"Written consolidated transcript: A11_all_channels.txt")
except Exception as e:
    print(f"Error writing consolidated transcript 'A11_all_channels.txt': {e}")
