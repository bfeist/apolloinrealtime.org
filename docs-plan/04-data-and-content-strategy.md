...
changing an adapter. React components consume these adapters through the
fetchers and TanStack Query hooks in `src/api/`. Query keys include the mission
ID; downloaded records belong in the query cache, not duplicated in Zustand.
Panel-specific sample and biometric hooks follow the same pattern beside
their components.
...
Do not regenerate historical content or rewrite preserved snapshots as part
of application work. A replacement pipeline needs its own verified input and
output contract, with output compared against all three current missions.
