...
/** Trusted, mission-specific copy preserved from the original MOCRviz About view. */
export function createMocrvizAbout(mission: MocrMissionId, mediaRoot: string): HTMLDivElement {
  const copy = ABOUT_COPY[mission];
  const imageRoot = `${mediaRoot.replace(/\/+$/, "")}/MOCRviz/img`;
  const about = document.createElement("div");
  about.className = "mocrviz-transcript-about";
  about.hidden = true;
  about.innerHTML = `
    <div class="title">About This Mission Control Audio</div>
    <div class="copy">
    ...
    <div class="subtitle">Digitization</div>
    <div class="copy">
      <p>
        The tapes containing this audio were digitized in a
        <a href="https://www.nasa.gov/feature/nasa-university-of-texas-at-dallas-reveal-apollo-11-behind-the-scenes-audio" target="_blank" rel="noopener noreferrer">multi-year effort</a>
      ...
      ${
        mission === "13"
          ? `<p>
              ...
              National Archives, having been separated from the others for many decades, due to
              their use in the post-mission congressional investigation tasked to find the cause
              of the on-board explosion. They were digitized by NASA's Johnson Space Center (JSC)
              in February and March, 2020 with the help of JSC Historical Preservation Officer,
              Sandra Tetley.
            </p>`
          : ""
      }
    </div>
    <div class="mocrviz-about-images">
      ${copy.images
        .map(
          (image) =>
            `<a href="${imageRoot}/${image}" target="_blank" rel="noopener noreferrer"><img src="${imageRoot}/${image}" height="70" alt="Apollo ${mission} Mission Control audio tape"></a>`,
        )
        .join("")}
    </div>
    ...
    <div class="copy">
      ...
      <p>
        ...
        drift that makes the sounds warble in pitch about three times a second) were corrected using
        a custom algorithm developed by Apollo in Real Time team member,
        <a href="https://twitter.com/ke6jjj" target="_blank" rel="noopener noreferrer">Jeremy Cooper</a>.
